...
def render(m: dict[str, Any], payload: dict, validation: dict, ex_stats: dict,
    ...
    for r in rows:
        ...
        seen.add(r["rule"])
        src_rows += (
            f'<tr><td><span class="chip">{html.escape(r["source_app"]).upper()}</span></td>'
            f'<td class="mono">{html.escape(r["rule"])}</td>'
            f'<td>{html.escape(r["title"])}</td>'
            f'<td class="mono">{html.escape(r["grader"])}</td></tr>')
...
<div class="panel">
  ...
  <table class="t"><thead><tr><th>source</th><th>rule</th><th>failure</th><th>grader</th></tr></thead>
...
def write(m, payload, validation, ex_stats, candidates=None) -> str:
    path = RUNS / "report.html"
    path.write_text(render(m, payload, validation, ex_stats, candidates))
    return str(path)
